...
Write-Host "Qté de comptes concernés : $qteUsers" -ForegroundColor White
 "-------------------   Qté de comptes concernés : $qteUsers   ------------------" | Out-File ($DirPath + "\" + "Log.txt") -Append
...
foreach ($user in $users)
{
	...
	If (($daystoexpire -ge "0") -and ($daystoexpire -lt $expireindays))
	{
		...
		$mailmessage.Body =
		"Cher(e) $Name,
Le mot de passe utilisé pour se connecter à Windows arrive à expiration dans $daystoexpire jours.
Merci de penser à le changer dès que possible.

La procédure de changement du mot de passe est la suivante:
1. Depuis la session Windows 
	a.	Le client VPN Sophos doit être monté au préalable, si le pc n'est pas sur le réseau d'ICE. 
	b.	Ouvrir sa session comme d'habitude et s'assurer d'être connecté à Internet.
	c.	Pressez sur les touches Ctrl-Alt-Del simultanement and cliquez sur ""Modifier un mot de passe"".
	d.	Saisissez l'ancien mot de passe et définissez le nouveau en respectant les contraintes de complexité décrites plus bas.
	e.	Appuyez sur OK pour valider la modification. 

Le nouveau mot de passe doit répondre aux exigences minimales énoncées dans notre politique d'entreprise, notamment :
	1. Il doit comporter au moins 9 caractères.
	2. Il doit contenir au moins un caractère parmi 3 des 4 groupes de caractères suivants :
		a. Lettres majuscules (A-Z)
		b. Lettres minuscules (a-z)
		c. Chiffres (0-9)
		d. Symboles (!@#$%^&*...)
	3. Il ne peut correspondre à aucun de vos 24 derniers mots de passe.
	4. Il ne peut pas contenir de caractères correspondant à 3 caractères consécutifs ou plus de votre nom d'utilisateur.
	5. Vous ne pouvez pas changer votre mot de passe plus d'une fois par période de 24 heures.

Si vous avez des questions, vous pouvez contacter le service informatique à l'adresse support-it@ice-wm.com
